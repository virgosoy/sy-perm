...
export interface PermDb {
  ...
  /**
   * 根据子角色id获取所有父角色id
   * @param childrenRoleId 
   */
  listRoleIdByChildRoleId(childrenRoleId: Role['id']): Promise<Role['id'][]>
  /**
   * 根据子角色id获取所有父角色
   * @param childrenRoleId 
   * @implements 可选，如果没有会使用默认实现
   */
  listRoleByChildRoleId?(childrenRoleId: Role['id']): Promise<Role[]>
  /**
   * 重设连接关联关系，等同于移除全部关系后设置，角色与其父角色
   */
  setRoleExtendsRelation(roleId: Role['id'], parentIds: Role['id'][]): Promise<void>
  // /**
  //  * 新增连接关联关系，角色与其父角色。不会移除原有的关系。
  //  */
  // addRoleExtendsRelation(roleId: Role['id'], parentIds: Role['id'][]): Promise<void>
  // /**
  //  * 移除连接关联关系，角色与其父角色
  //  */
  // removeRoleExtendsRelation(roleId: Role['id'], parentIds: Role['id'][]): Promise<void>
...
export async function listRoleIdByChildRoleId(childrenRoleId: Role['id']) {
  return _db.listRoleIdByChildRoleId(childrenRoleId)
}
 ...
 * @param childrenRoleId 
 */
export async function listRoleByChildRoleId(childrenRoleId: Role['id']) {
  if(_db.listRoleByChildRoleId){
    return _db.listRoleByChildRoleId(childrenRoleId)
  }
  // 默认实现：
  // return _db.listRoleByIds(await listRoleIdByExtendRoleId(childrenRoleId))
  const parentIds = await listRoleIdByChildRoleId(childrenRoleId)
  const parents = (await listRole()).filter(role => parentIds.includes(role.id))
  return parents
}
...
export async function setRoleExtendsRelation(roleId: Role['id'], parentIds: Role['id'][]) {
  await _db.setRoleExtendsRelation(roleId, parentIds)
}

// /**
//  * 新增连接关联关系，角色与其父角色。不会移除原有的关系。
//  * 暂时使用 setRoleExtendsRelation 进行代替
//  */
// export async function addRoleExtendsRelation(roleId: Role['id'], parentIds: Role['id'][]) {
//   await _db.addRoleExtendsRelation(roleId, parentIds)
// }
...
//   await _db.removeRoleExtendsRelation(roleId, parentIds)
// }
